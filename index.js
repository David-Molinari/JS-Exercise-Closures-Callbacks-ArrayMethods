...
function processLastItem(stringList, callback) {
  let listIndex = stringList.length - 1;
  return callback(stringList[listIndex])
}
...
function processProduct(num1, num2, callback) {
  let numProd = num1 * num2;
  return callback(numProd)
}
...
function getFullNames(runners) {
  ...
  runners.forEach(function(item) {
    return nameListing.push(`${item.last_name}, ${item.first_name}`);
});
return nameListing
}
...
function firstNamesAllCaps(runners) {
  const firstNames = runners.map((state) => {
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const tShirtSizeFiltered = runners.filter((state) => {
    return state.shirt_size == tShirtSize
  })
  return tShirtSizeFiltered
}
...
function tallyUpDonations(runners) {
  const runnersDonations = runners.reduce((total, state) => {
    return total += state.donation;
  }, 0);
  return runnersDonations
}
